...
import com.example.android.codelabs.paging.data.remote.IN_QUALIFIER
import com.example.android.codelabs.paging.data.local.keys.RemoteKeys
    ...
    override suspend fun load(loadType: LoadType, state: PagingState<Int, Repo>): MediatorResult {
        ...
        try {
            ...
            if (loadType == LoadType.REFRESH) {
                keysRepository.clearRemoteKeys()
                repoRepository.clearRepos()
            }
            ...
            keysRepository.insertAll(keys)
            repoRepository.insertAll(repos)
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, Repo>): RemoteKeys? {
        ...
        return state.pages.lastOrNull() { it.data.isNotEmpty() }?.data?.lastOrNull()
            ?.let { repo ->
                ...
                keysRepository.remoteKeysRepoId(repo.id)
            }
    }
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(
        ...
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.id?.let { repoId ->
                keysRepository.remoteKeysRepoId(repoId)
            }
        }
    }
}
