...
val viewModelModule = module {
    ...
    fun provideUseCase(repository: ContactRepository): UseCases = UseCases(
        ...
        GetAllContactListFromDBUseCase(repository = repository),
        DeleteContactByIdUseCase(repository = repository),
        DeleteContactUseCase(repository = repository)
    )
    ...
    viewModel {
        ContactsListViewModel(
            ...
            provideUseCase(provideRepository(get(), get())).getAllContactListFromDBUseCase,
            provideUseCase(provideRepository(get(), get())).deleteContactByIdUseCase,
            provideUseCase(provideRepository(get(), get())).deleteContactUseCases
        )
    }
}
