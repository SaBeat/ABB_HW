    ...
    operator fun invoke(fileUrl: String): Flow<Resource<String>> = flow {
        ...
        try {
            ...
            while (currentRead != -1 && currentCoroutineContext().isActive) {
                ...
                val size = (downloadedFileSize*100) / buffer.size
                valuePercent = ((size*100)/buffer.size).toFloat()

                // for testing only
                Log.d("DownloadingTest", "chunk $currentRead")
                Log.d("TAG", "Downloading file size ---->   $valuePercent")
                emit(Resource.Loading<String>("",valuePercent))
                delay(1000)
                // for testing only
            }
